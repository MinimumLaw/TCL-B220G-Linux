...
Я сделал ISO образ Windows 11 Home 24H2 (сборка 26100.268). Прямая ссылка для загрузки https://www.mediafire.com/file/892ad7p4u3tpcd7/TCL-B220G-Windows11-Home-RU-24H2-26100.268.240408.iso/file ISO'шка сделана с промощью UUPDump - https://uupdump.net/selectlang.php?id=94cce692-a3e0-4642-b97e-e508766dafce. Это обычная сборка для arm64, с поддержкой русского языка и без каких-то изменений, типа отключения сетевых учетных записей или удаления Microsoft Edge. Т.е. по сути это чистый образ. Все, что в нём поменялось - это в него интегрированы драйвера (в оба образа - и системный и инсталляционный). Драйвера "выдернуты" из рабочей системы с помощью DISM. По непонятным причинам попытка интегрировать в образ драйвера от производителя заканчивалась провалом - образ не работал. На своем ноутбуке я сборку проверил - она сработала. При установке работал трекпад, клавиатура, беспороводные интерфейсы (включая мобильную сеть). По непонятным причинам, для загрузки с флешки необходимо отключить SecureBoot и включить Developer Mode в BIOS, после чего выбрать "Save and reboot". После этого при перезагрузке можно нажать F7 и выбрать загрузку с флешки (создданной из данной ISO'шки c помощью rufus'а - https://rufus.ie/ru/) или внешнего DVD-ROM, подключенного по USB. После установки можно отключить DeveloperMode и включить SecureBoot. Надеюсь, наличие такой флешки позволит более смело заниматься попытками установки альтернативных операционных систем (и в первую очередь Linux). К сожалению я в этом направлении особенно не продвинулся.

@nektovse сделал инсталляционный образ для Windows 11 IoT для версии 8/256Gb взяв MSDN образы Microsoft Windows 11 IoT Enterprise/LTSC 2024 version 24H2 build 26100.1742 и интегрировав в них драйвера с помощью DISM. Ссылка для скачивания - https://mega.nz/file/XFkyBArC#W8JyTMwCQsizVYznRDcSQhibUG6ilJHML16lGLwwdxA С его слов различий в производительности не замечено, но LTSC IoT редакция содержит сильно меньше ненужного. Вопросы лицензирования каждый решает сам. В MSDM таблице BIOS ревизия другая. 

Имеется информация о возможном аппаратном моддинге данного ноутбука, устаняющем одну из самых неприятных особенностей. А именно очень посредственный диcплей ([источник](https://forum.ixbt.com/topic.cgi?id=17:57666:22#22)). Part Number и производитель матрицы для замены - BOE TV140FHM-NH1 В результате получаем 1920x1080"60Гц, 157PPI со значительно лучшим качеством картинки.
